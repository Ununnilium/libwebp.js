...
libwebp-asm.js: emsdk libwebp
	cd src && source ../emsdk/emsdk_env.sh && emcc -Oz --closure 1 --llvm-lto 1 -o ../libwebp-asm.js --memory-init-file 0 \
		...
		-s ALLOW_MEMORY_GROWTH=1 -s ENVIRONMENT=web -s NO_FILESYSTEM=1 -s "EXPORTED_FUNCTIONS=['_free', '_malloc']" \
		-I ../libwebp \
		webp.c \
...
libwebp-wasm.js: emsdk libwebp
	cd src && source ../emsdk/emsdk_env.sh && emcc -Oz --closure 1 --llvm-lto 1 -o ../libwebp-wasm.js \
		...
		webp.c \
		../libwebp/src/{dec,dsp,demux,enc,mux,utils}/*.c
	sed -i 's!libwebp-wasm.wasm!static/libwebp-wasm.wasm!' libwebp-wasm.js
emsdk:
...
install: libwebp-asm.js libwebp-wasm.js webp-polyfill.js webp-polyfill.min.js
	mkdir -p lib
